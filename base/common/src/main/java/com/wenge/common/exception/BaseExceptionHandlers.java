...
public class BaseExceptionHandlers {
	...
	private final Map<Class<? extends Exception>, HttpStatus> HTTP_STATUS = CommonUtil.ofMapN(
	...
	public ResponseEntity<Result<?>> fail(Exception exception, String message, Integer failType) {
	...
	private ResponseEntity<Result<?>> fail(Exception exception, String message, String code, String[] args, Integer failType) {
		...
		HttpStatus status = this.HTTP_STATUS.get(exception.getClass());
		if (status == null) {
	...
	public void add(Class<? extends Exception> clazz, HttpStatus httpStatus) {
		this.HTTP_STATUS.put(clazz, httpStatus);
	}
